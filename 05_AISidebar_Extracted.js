// Extracted functions from 05_AISidebar_UI.js
// These functions are not used in the showAISidebar() execution flow
...
function addCommercialFitSkuVariant(request) {
  ...
  const validation = validateCommercialFitRow(scopeEntryId, 'SelectedSkusJSON');
  if (!validation.ok) {
    return { ok: false, error: validation.error };
  }
  const rowInfo = validation.rowInfo;
  const selectedIdx = validation.columnIdx;
...
function addCommercialFitVectorCandidate(request) {
  ...
  const validation = validateCommercialFitRow(scopeEntryId, 'VectorCandidatesJSON');
  if (!validation.ok) {
    return { ok: false, error: validation.error };
  }
  const rowInfo = validation.rowInfo;
  const vectorIdx = validation.columnIdx;
...
function removeCommercialFitEntry(request) {
  ...
  try {
    const sheetResult = getCommercialFitStateSheet();
    const validation = validateSheetDimensions(sheetResult);
    if (!validation.ok) {
      return { ok: false, error: validation.error };
    }
...
function persistSidebarState(changes) {
  ...
  if (updates.activeSnapshotId) {
    ...
    getSidebarHistory(user, 'snapshot', 25).forEach(function(item) {
  ...
  if (updates.acknowledgeRunId) {
    ...
    getSidebarHistory(user, 'quote_run', 50).forEach(function(item) {
      const payload = Object.assign({}, item.data || {});
      payload.id = payload.id || item.id;
...
function escapeJsonStringNewlines(text) {
  return processJsonString(text, function(ch, i, inString, fullText) {
    if (inString && (ch === '\n' || ch === '\r')) {
...
function removeEllipsisPlaceholders(text) {
  return processJsonString(text, function(ch, i, inString, fullText) {
    if (!inString) {
      if (ch === '.' && fullText[i + 1] === '.' && fullText[i + 2] === '.') {
        let skip = 0;
        while (i + skip < fullText.length && fullText[i + skip] === '.') {
          skip++;
        }
        return { value: '', skipChars: skip - 1 };
      }
      if (ch === '\u2026') {
        return '';
      }
...
function stripTrailingCommas(text) {
  return processJsonString(text, function(ch, i, inString, fullText) {
    if (ch === ',' && !inString) {
      ...
      if (j < fullText.length && (fullText[j] === '}' || fullText[j] === ']')) {
        return { value: '', skipChars: j - i - 1 };
      }
    }
    return ch;
  });
